...
# Config la ddb
mongo = config_db(app, "myDatabase")
...
def user_msg_stats():
    ...
    df_users["messages"] = df_users["user"].apply(
        lambda user: list(df.loc[df["user"] == user]["messages"])[0] if user in df["user"].values else 0)
...
def server_msg_stats():
    ...
    df_servers["messages"] = df_servers["server"].apply(
        lambda server: list(df.loc[df["server"] == server]["messages"])[0] if server in df["server"].values else 0)
